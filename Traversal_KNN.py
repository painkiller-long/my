...
def knn_traversal_train():
    traversal_matrix = generate_traversal("./data/knn/traversal/traversal_train.csv", "./data/knn/traversal/traversal_matrix.csv", 1)
    traversal_normal_matrix = generate_traversal("./data/knn/traversal/normal_train.csv", "./data/knn/traversal/traversal_normal_matrix.csv", 0)

    # 将两个csv文件合并在一个csv文件中
    df = pd.read_csv(traversal_matrix)                                                   # 读取CSV格式的特征矩阵文件，并将其存储到一个名为df的Pandas数据框中
    df.to_csv("./data/knn/traversal/traversal_all_matrix.csv", encoding="utf_8_sig", index=False)      # _sig表示该编码格式带有 BOM（Byte Order Mark）头，用于在文本编辑器中正确显示中文字符
    df = pd.read_csv(traversal_normal_matrix)
    df.to_csv("./data/knn/traversal/traversal_all_matrix.csv", encoding="utf_8_sig", index=False, header=False, mode='a+')

    feature_max = pd.read_csv('./data/knn/traversal/traversal_all_matrix.csv')
    ...
    joblib.dump(knn, './model/knn_traversal.model')                         # 将训练好的 knn k-近邻分类器模型保存到指定路径下的 knn.model 文件中，以便之后可以快速地加载模型进行预测
    # print("knn_traversal model has been saved to 'model/knn_traversal.model'")
