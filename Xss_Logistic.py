...
def logistic_xss_train():
    xss_matrix = generate_xss("./data/logistic/xss/xss_train.csv", "./data/logistic/xss/xss_matrix.csv", 1)
    xss_normal_matrix = generate_xss("./data/logistic/xss/normal_train.csv", "./data/logistic/xss/xss_normal_matrix.csv", 0)

    df = pd.read_csv(xss_matrix)
    df.to_csv("./data/logistic/xss/all_matrix.csv", encoding="utf_8_sig", index=False)
    df = pd.read_csv(xss_normal_matrix)
    df.to_csv("./data/logistic/xss/all_matrix.csv", encoding="utf_8_sig", index=False, header=False, mode='a+')

    feature_max = pd.read_csv('./data/logistic/xss/all_matrix.csv')
    ...
    joblib.dump(logistic, './model/logistic_xss.model')
    # print("logistic_xss model has been saved to 'model/logistic_xss.model'")
