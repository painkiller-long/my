...
def logistic_sql_train():
    sql_matrix = generate_sql("./data/logistic/sql/sql_train.csv", "./data/logistic/sql/sql_matrix.csv", 1)
    sql_normal_matrix = generate_sql("./data/logistic/sql/normal_train.csv", "./data/logistic/sql/sql_normal_matrix.csv", 0)

    df = pd.read_csv(sql_matrix)
    df.to_csv("./data/logistic/sql/all_matrix.csv", encoding="utf_8_sig", index=False)
    df = pd.read_csv(sql_normal_matrix)
    df.to_csv("./data/logistic/sql/all_matrix.csv", encoding="utf_8_sig", index=False, header=False, mode='a+')

    feature_max = pd.read_csv('./data/logistic/sql/all_matrix.csv')
    ...
    joblib.dump(logistic, './model/logistic_sql.model')
    # print("logistic_sql model has been saved to 'model/logistic_sql.model'")
    y_pred = logistic.predict(test_data)    # 预测
    # print("y_pred:%s" % y_pred)
    # print("test_target:%s" % test_target)
    # Verify
    print("*" * 42)
    print("逻辑回归模型SQL注入攻击训练结果报告")
