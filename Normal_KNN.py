...
def knn_normal_train():
    normal_matrix = generate_abnormal("./data/knn/normal/normal_train.csv", "./data/knn/normal/normal_matrix.csv", 1)
    abnormal_matrix = generate_abnormal("./data/knn/normal/abnormal_train.csv", "./data/knn/normal"
                                                                                "/abnormal_matrix.csv", 0)

    # 将两个csv文件合并在一个csv文件中
    df = pd.read_csv(abnormal_matrix)  # 读取CSV格式的特征矩阵文件，并将其存储到一个名为df的Pandas数据框中
    df.to_csv("./data/knn/normal/normal_all_matrix.csv", encoding="utf_8_sig",
              index=False)  # _sig表示该编码格式带有 BOM（Byte Order Mark）头，用于在文本编辑器中正确显示中文字符
    df = pd.read_csv(normal_matrix)
    df.to_csv("./data/knn/normal/normal_all_matrix.csv", encoding="utf_8_sig", index=False, header=False, mode='a+')

    feature_max = pd.read_csv('./data/knn/normal/normal_all_matrix.csv')
    arr = feature_max.values  # 使用values属性将feature_max数据框转换为一个NumPy数组arr，其中每行代表一个样本的特征向量
    data = np.delete(arr, -1, axis=1)  # 删除最后一列
    # print(arr)
    target = arr[:, 13]  # 将arr数组的第7列（即标签列）提取出来
    # 随机划分训练集和测试集
    ...
    joblib.dump(knn, './model/knn_normal.model')  # 将训练好的 knn k-近邻分类器模型保存到指定路径下的 knn.model 文件中，以便之后可以快速地加载模型进行预测
    # print("knn_normal model has been saved to 'model/knn_normal.model'")
