...
def knn_crlf_train():
    crlf_matrix = generate_crlf("./data/knn/crlf/crlf_train.csv", "./data/knn/crlf/crlf_matrix.csv", 1)
    crlf_normal_matrix = generate_crlf("./data/knn/crlf/normal_train.csv", "./data/knn/crlf/crlf_normal_matrix.csv", 0)

    # 将两个csv文件合并在一个csv文件中
    df = pd.read_csv(crlf_matrix)                         # 读取CSV格式的特征矩阵文件，并将其存储到一个名为df的Pandas数据框中
    df.to_csv("./data/knn/crlf/crlf_all_matrix.csv", encoding="utf_8_sig", index=False)      #_sig表示该编码格式带有 BOM（Byte Order Mark）头，用于在文本编辑器中正确显示中文字符
    df = pd.read_csv(crlf_normal_matrix)
    df.to_csv("./data/knn/crlf/crlf_all_matrix.csv", encoding="utf_8_sig", index=False, header=False, mode='a+')

    feature_max = pd.read_csv('./data/knn/crlf/crlf_all_matrix.csv')
    ...
    joblib.dump(knn, './model/knn_crlf.model')     # 将训练好的 knn k-近邻分类器模型保存到指定路径下的 knn.model 文件中，以便之后可以快速地加载模型进行预测
    # print("knn_crlf model has been saved to 'model/knn_crlf.model'")
    y_pred = knn.predict(test_data)                               # y_pred 是一个一维数组，包含了测试数据中每个样本的预测结果
    # Verify
    print("*" * 42)
    print("KNN模型CRLF注入攻击训练结果报告")
